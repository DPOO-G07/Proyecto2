...
public class control {
	...
	public void reservar() throws ParseException{
			...
			JOptionPane.showMessageDialog(inter,"Se realizo con exito la reserva y el cobro del 30%, puede pasar el : " + fechadeRecoleccion + " a las" + horadeRecoleccion + " a la sede " + sede + " para poder recoger su carro, el numero de reserva es: "+ id, "Exito" ,JOptionPane.INFORMATION_MESSAGE);
	...
	public void recoger() throws ParseException {
		...
		if ("si".equals(conductorextra)) {
			double id =  Double.parseDouble(JOptionPane.showInputDialog("Por favor ingrese el numero de la reserva y a continuacion se le pediran los datos para el otro conductor :"));
			Inscripcion("Cliente");
			double cobro = ren.obtenercobrofinal(id);
			double cobroconotroconductor = cobro*0.2 + cobro;
			String seguroAd = JOptionPane.showInputDialog("El cobro de un conductor adicional es de: " + cobro*0.2 + " Es decir el cobro total seria de: " + cobroconotroconductor + "\nDeseea incluir un seguro adiciona responda si/no");
			if ("si".equals(seguroAd)) {
				...
				double numerotar = Double.parseDouble(JOptionPane.showInputDialog("El costo de un dia para el seguro es de" + costoundiaseguro + "por esto el cobro total con el seguro para todos los dias es de " + (costototalseguro+cobroconotroconductor) + " Por favor ingrese el numero de su tarjeta que utilizo para pagar la reserva:"));
				ren.cambiarestadotarjeta(numerotar);
				JOptionPane.showMessageDialog(inter,"Se realizo el cobro total, ya puede recoger su vehiculo y su tarjeta ha sido bloqueada hasta que se devuelva el vehiculo" ,"Exito", JOptionPane.INFORMATION_MESSAGE);
